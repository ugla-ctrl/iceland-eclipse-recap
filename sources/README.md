...
5. **Design language (2026-09-03 redesign, per Mitch's "should not look exactly like MSF" direction):** dark totality aesthetic built from Iceland Eclipse's own existing brand, specifically the IEpress press page (thebaileyperspective.com/IEpress) palette: bg `#04060a`, panel `#0b1118`, line `#15222d`, ink `#eef4f2`, dim `#8fa3ab`, teal `#0DFCD3`, corona orange `#EE7717`. Type: Unbounded (display) + IBM Plex Mono (labels/data) + Inter (body). Corona-glow logo mark (breathing animation, disabled under prefers-reduced-motion), eclipse-styled nav dots (active dot = dark disc with orange ring). Only the slide *engine* (keyboard/swipe/wheel/tap JS) is shared with the other decks; the visual system is Iceland Eclipse's own. Do not revert it to the light pastel Midnight Sun look.
